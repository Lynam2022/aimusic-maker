...
import { Music, Eye, EyeOff, Loader2, X, CheckCircle2, AlertCircle, Sparkles } from 'lucide-react';

export default function AuthModal() {
  const { showAuthModal, setShowAuthModal, authModalTab, setAuthModalTab } = useMusicStore();
  ...
  // Clear state when modal closes/opens & read notice
  useEffect(() => {
    if (showAuthModal) {
      const storedNotice = localStorage.getItem('auth_modal_notice');
      if (storedNotice) {
        setNotice(storedNotice);
        localStorage.removeItem('auth_modal_notice');
      }
    } else {
      setName('');
      setEmail('');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal Box */}
      <div className="relative w-full max-w-md bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-6 md:p-8 shadow-2xl z-10 animate-scale-up">
        {/* Close Button */}
        <button
          onClick={() => setShowAuthModal(false)}
          className="absolute right-4 top-4 p-1.5 rounded-lg text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)] transition-all border border-transparent hover:border-[var(--border)] cursor-pointer"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="flex flex-col items-center mb-6">
          <div className="h-11 w-11 rounded-xl bg-[var(--accent-dim)] border border-[var(--accent)]/30 flex items-center justify-center mb-2">
            <Sparkles className="h-5 w-5 text-[var(--accent)]" />
          </div>
          <h2 className="text-lg font-black text-[var(--text-primary)] tracking-wide">
        ...
        <div className="flex border-b border-[var(--border)] mb-5">
          <button
            onClick={() => { setAuthModalTab('login'); setError(''); setSuccess(''); }}
            className={`flex-1 pb-2.5 text-xs font-bold border-b-2 transition-all cursor-pointer ${
              authModalTab === 'login'
                ? 'border-[var(--accent)] text-[var(--text-primary)] font-black'
          ...
          <button
            onClick={() => { setAuthModalTab('register'); setError(''); setSuccess(''); }}
            className={`flex-1 pb-2.5 text-xs font-bold border-b-2 transition-all cursor-pointer ${
        ...
        {notice && !error && !success && (
          <div className="mb-4 p-3.5 rounded-xl bg-[var(--accent-dim)] border border-[var(--accent)]/40 text-[var(--text-primary)] text-xs font-semibold leading-relaxed flex items-center gap-3 shadow-sm animate-fade-in">
            <div className="h-6 w-6 rounded-full bg-[var(--accent)]/20 flex items-center justify-center flex-shrink-0">
        ...
        >
          ...
          <span>
            {authModalTab === 'login' ? 'Đăng nhập bằng Google' : 'Đăng ký bằng Google'}
          </span>
        </button>
      </div>
    </div>
